---
title: "Paper Review: Reconstructing the cascade of language processing in the brain using the internal computations of a transformer-based language model"
date: "July 6, 2022"
---
## What?
There's been some work done in trying to connect linguistic representations in BERT and language-related cortices in the brain. Because BERT is simply a number of Transformer encoders stacked sequentially with an output layer determined by the specific task the model is used for and Transformers go through multiple steps of computations within the encoder, multiple representations are generated for the same input token. While most papers focus on the embeddings, which are the outputs of each encoder layer, this paper decided to focus on what they denote as the transformations, which are the outputs of the self-attention layer in the encoder model (a depiction of the internals of the encoder model is in the slides). 
...
## Why?
I'm currently working at [Cai Lab](https://cailab-ircn.org/) under IRCN over the summer. We have weekly meetings where a lab member gives a 2 hour presentation, and I happened to have the chance to present a paper on the connection between linguistic computations in the brain and representations with BERT. 
